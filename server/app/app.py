...
async def connect(sid, environ):
    await sio.emit('response', {'message': f'SERVER: Connected as {bot.nickname}'}, room=sid)
...
async def message(sid, data):
    ...
    if bot.send_message(data['data']):
        await sio.emit('response', {'message': f"Message sent: {data['data']}"}, room=sid)
    else:
        await sio.emit('response', {'message': "Failed to send message. Bot not connected."}, room=sid)
